    ...
    $(document).bind("mobileinit", function () {
        ...
        $.extend($.mobile, {
            jqmNavigator:{
                ...
                /**
                 * Show the dialog and this not added into the stack, once dialog closed its been removed from DOM
                 *
                 * @param view {Backbone.View} as Dialog
                 * @param options {*} Transition parameters can be passed like: transition, reverse, showLoadMsg or loadMsgDelay
                 */
                showDialog: function jqmNavigator_showDialog(dialog, options) {
                    var containerViews = this._getPageContainerViews(options);
                    containerViews.pageContainer.append(dialog.el);
                    dialog.$el.attr('data-role', 'dialog');
                    dialog.render();

                    dialog.$el.one('pagehide', function (event) {
                        // Detaching view from DOM
                        dialog.$el.detach();
                    });

                    $.mobile.changePage(dialog.$el, $.extend({
                        role: "dialog",
                        changeHash: false,
